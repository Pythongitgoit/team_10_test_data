...
def sort_files(folder_path):
    ...
    check_folder(folder, known_extension, unknown_extension, folder / 'archives', dict_extension)
    items_to_sort = collect_files_and_folders(folder, excluded_folders)
    sort_files_by_extension(items_to_sort, dict_extension, known_extension, unknown_extension, folder)
...
def sort_files_by_extension(items, dict_extension, known_extension, unknown_extension, folder):
    for item in items:
        if item.is_file() and not item.name.startswith('.DS_Store'):
            ...
            if not sorted:
                ...
                unknown_folder_path = folder / 'unknown'
